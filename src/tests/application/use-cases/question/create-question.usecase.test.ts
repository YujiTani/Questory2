...
describe("create-question.usecase.tsのテスト", () => {
  ...
  test("カテゴリー:HTTP、タイプ:MULTIPLE_CHOICEで問題を作成できる",  async () => {
    ...
    const result = await createQuestionUseCase.execute(
      ...
      explanation: "HTTPステータスコードは3桁の数字で、最初の数字によって5つのクラスに分類されます。「4xx」で始まるコードはクライアントエラーを示し、「404 Not Found」はリソースが見つからない場合、「400 Bad Request」はリクエストの構文が不正な場合に返されます。一方、「2xx」は成功、「3xx」はリダイレクト、「5xx」はサーバーエラーを示します。",
